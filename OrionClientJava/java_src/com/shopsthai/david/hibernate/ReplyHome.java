...
/**
 * Home object for domain model class Reply.
 * @see com.shopsthai.david.hibernate.Reply
 * @author Hibernate Tools
 */
public class ReplyHome {

	private static final Log log = LogFactory.getLog(ReplyHome.class);
	...
	public void persist(Reply transientInstance) {
		log.debug("persisting Reply instance");
		try {
	...
	public Reply merge(Reply detachedInstance) {
		...
		try {
			Reply result = (Reply) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Reply findById(java.lang.Long id) {
		...
		try {
			Reply instance = (Reply) sessionFactory.getCurrentSession().get(
					"com.shopsthai.david.hibernate.Reply", id);
			if (instance == null) {
	...
	public List<Reply> findByExample(Reply instance) {
		...
		try {
			List<Reply> results = (List<Reply>) sessionFactory
					.getCurrentSession()
					.createCriteria("com.shopsthai.david.hibernate.Reply")
