 ...
 * @see com.shopsthai.david.hibernate.Friend
 * @author Hibernate Tools
 */
public class FriendHome {

	private static final Log log = LogFactory.getLog(FriendHome.class);
	...
	public Friend merge(Friend detachedInstance) {
		...
		try {
			Friend result = (Friend) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Friend findById(java.lang.Integer id) {
		...
		try {
			Friend instance = (Friend) sessionFactory.getCurrentSession().get(
					"com.shopsthai.david.hibernate.Friend", id);
			if (instance == null) {
	...
	public List<Friend> findByExample(Friend instance) {
		...
		try {
			List<Friend> results = (List<Friend>) sessionFactory
					.getCurrentSession()
					.createCriteria("com.shopsthai.david.hibernate.Friend")
