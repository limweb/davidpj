package custom_tree_component
{
	...
	public class CheckTreeRenderer_backup1112013 extends TreeItemRenderer
	{
		...
		public function CheckTreeRenderer_backup1112013()
		{
			super();
			mouseEnabled=false;
		}
		...
		private function toggleParents(item:Object, tree:Tree, state:String):void
		{
			...
			{
				//item.@state=state; //Cee comment 20131203
				item.state=state;
				
				//setCheckState(myCheckBox, value, value.@state);
				//setCheckState(myCheckBox, data, state);
		...
		private function getState(tree:Tree, parent:Object):String
		{
			...
			if (parent != null)
			{
				...
				while (!cursor.afterLast)
				{
					//if (cursor.current.@state == STATE_CHECKED) Cee comment 20131203
					if (cursor.current.state == STATE_CHECKED)
					{
						noChecks++;
					}
					//else if (cursor.current.@state == STATE_UNCHECKED) Cee comment 20131203
		...
		private function checkBoxToggleHandler(event:MouseEvent):void
		{
			if (data)
			{
				...
				FlexGlobals.topLevelApplication.caseManagerID.updateTreeList(tree);
		...
		//Cee 08/12/2013
		private function setAllTreeChkBlox( ):void {
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			if (super.data)
			{
				...
				//if (data.@state == STATE_SCHRODINGER) Cee comment 20131203
