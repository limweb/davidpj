package flex.utils.spark.resize {
	 ...
	 */
	public class ResizeManagerL extends EventDispatcher {
		...
		public function ResizeManagerL(resizeComponent:UIComponent = null, resizeHandle:UIComponent = null, resizeDirection:String = "both") {
			this._enabled = true;
		...
		private function startResize(globalX:Number, globalY:Number):void {
			...
			if (okay) {
				...
				if (bringToFrontOnResize && resizeComponent.parent) {
					trace('parent',flash.utils.getQualifiedClassName(resizeComponent.parent),'owner',flash.utils.getQualifiedClassName(resizeComponent.owner));
				...
				resizeInitX = globalX; trace('globalx =',globalX,'global y=',globalY);
				startWidth = resizeComponent.width;
				startHeight = resizeComponent.height;
				resizeInitY = globalY; trace('startwidth',startWidth,'startheight',startHeight);
				trace('parent',flash.utils.getQualifiedClassName(resizeComponent.parent),'owner',flash.utils.getQualifiedClassName(resizeComponent.owner));
		...
		private function resizeMouseMoveHandler(event:MouseEvent):void {
			...
			trace('event.stagex',event.stageX,'resizeInitx',resizeInitX);
//			var newWidth:Number = oldWidth + event.stageX - resizeInitX;
			var newWidth:Number = oldWidth + resizeInitX - event.stageX;
			...
			if ((newWidth >= resizeComponent.minWidth) && (newWidth <= resizeComponent.maxWidth) && resizeH && getAllwidth()) {

				resizeComponent.x =   resizeComponent.x - (newWidth - oldWidth);
				trace('new x',resizeComponent.x,resizeComponent.x - (newWidth - oldWidth));
					resizeComponent.width = newWidth;
			} else {
				resizeComponent.width = defaultWidth;
			}
		...
		private function getAllwidth():Boolean {
			var idx:int = IVisualElementContainer(resizeComponent.owner).numElements;
			var ow:Number = resizeComponent.owner.width;
			var w:Number = 0;
			for (var i:int = 0; i < idx; i++) 
			{
				w += IVisualElementContainer(resizeComponent.owner).getElementAt(i).width;
			}
			...
			if(ow >= w ){
				chk = true;
				defaultWidth = resizeComponent.width;
			}

			trace("LLLLLLLLLLLL",ow,w,chk);
			return chk;
			
			//				th.co.shopsthai.ascomponent::rzmvasBroderContainer
		}
